...
-- Your other inventory server functions like:
-- CreateNewDrop(...)
-- OpenInventory(...)

-- 🔽 PLACE THE FOLLOWING HERE:

-- Register the server event to reset the inv_busy flag
RegisterNetEvent("ps-inventory:server:closeinv", function()
    local src = source
    local ply = Player(src)
    if ply then
        ply.state.inv_busy = false
    end
end)

-- Function to open another player's inventory by their ID (used in robbing)
local function OpenInventoryById(source, targetId)
    ...
    -- Close existing inventory if already open
    if target.state.inv_busy then
        TriggerClientEvent("ps-inventory:client:closeinv", targetId)
    end

    Wait(500)
    target.state.inv_busy = true

    -- Open their inventory for the robber
    OpenInventory("otherplayer", targetId, nil, source)

    -- Auto-unlock after 30s to prevent softlock
    SetTimeout(30000, function()
        if target.state.inv_busy then
            target.state.inv_busy = false
        end
    end)
end
...
-- Injected Pickup Logic

-- Pickup drop handler
RegisterNetEvent("ps-inventory:server:PickupDrop", function(dropId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Drops[dropId] and Drops[dropId].items then
        for _, item in pairs(Drops[dropId].items) do
            Player.Functions.AddItem(item.name, item.amount, false, item.info)
        end
        Drops[dropId] = nil
        TriggerClientEvent("ps-inventory:client:RemoveDropItem", -1, dropId)
    end
end)
