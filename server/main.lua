...
local function AddItem(source, item, amount, slot, info, created, reason)
	local Player = QBCore.Functions.GetPlayer(source)
	...
	itemInfo['created'] = type(created) == 'number' and created or time
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData["created"], 'ps-inventory:server:SetInventoryData')
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
                AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info, itemInfo["created"], 'ps-inventory:server:SetInventoryData')
